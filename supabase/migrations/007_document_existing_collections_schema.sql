-- =============================================================
-- Documents schema that already exists live (applied 2026-07-02
-- via Supabase dashboard as migration "003_collections", outside
-- this repo's git history). Captured here so it's version-controlled
-- going forward. All statements are idempotent (IF NOT EXISTS /
-- ON CONFLICT DO NOTHING) — safe to run against an environment that
-- already has this schema, and safe for a fresh environment that
-- doesn't.
--
-- Design already in place (richer than originally planned):
-- `collections.storage` distinguishes 'generic' (entries live in
-- `collection_items.data` JSONB) from 'native' (the collection is a
-- view over an existing table — posts/pages/properties — via
-- `native_table`). `collections.client_id` may be NULL for a
-- global/system collection template, publicly readable by anyone
-- (`collections_global_read`).
-- =============================================================
...
-- NOTE: none of the three tables above currently have an updated_at
-- trigger live (confirmed via information_schema.triggers on
-- 2026-07-03), unlike posts/pages/properties/clients/collections'
-- sibling tables. Add one here for consistency with the rest of the
-- schema, reusing the existing trigger function from 001_initial_schema.sql.
